#!/usr/bin/env python3
"""DreamBooth dual-phase LoRA training for Stable Diffusion.

Phase 2 training that loads frozen phase 1 LoRA weights and trains
a second set of LoRA adapters on top. Useful for teaching a model
body structure after phase 1 learned facial features.

Usage:
    python src/train_dreambooth.py --config configs/training_config.yaml
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="DreamBooth dual-phase LoRA training")
    parser.add_argument(
...
def setup_dual_phase(cfg: dict[str, Any], accelerator: Accelerator) -> tuple:
    """Set up dual-phase LoRA pipeline.
    ...
    # Load phase 1 config for rank/alpha
    phase1_cfg = load_config(p2["phase1_config_path"]) if p2.get("phase1_config_path") else cfg
    r1 = phase1_cfg.get("lora_r", cfg["lora_r"])
    a1 = phase1_cfg.get("lora_alpha", cfg["lora_alpha"])

    # Dual-phase patching
    lora_params = patch_unet_dual_phase(unet, r1, a1, cfg["lora_r"], cfg["lora_alpha"])

    if cfg.get("train_text_encoder", False):
        lora_params += patch_text_encoder_dual_phase(
            pipe.text_encoder, r1, a1, cfg["lora_r"], cfg["lora_alpha"],
        )

    # Load phase 1 weights
    load_phase1_weights(unet, pipe.text_encoder, p2["phase1_lora_path"])

    if not lora_params:
        raise RuntimeError("No trainable phase 2 LoRA parameters collected.")

    print(f"Phase 2 trainable params: {sum(p.numel() for p in lora_params):,}")
...
def train(cfg: dict[str, Any]) -> None:
    """Run DreamBooth phase 2 training loop."""
    enable_tf32()
    ...
    pipe, unet, lora_params, tokenizer = setup_dual_phase(cfg, accelerator)
    ...
    lr_scheduler = get_scheduler(
        ...
        num_training_steps=p2["max_train_steps"],
    )

    dataset = ImageCaptionDataset(p2["dataset_dir"], tokenizer, cfg.get("resolution", 512))
    dataloader = DataLoader(dataset, batch_size=cfg.get("batch_size", 1), shuffle=True)

    unet.train()
    pipe.text_encoder.train()
    pipe.text_encoder, unet, optimizer, dataloader = accelerator.prepare(
        pipe.text_encoder, unet, optimizer, dataloader,
    )

    check_nan_params(unet, "UNet (pre-training)")

    global_step = 0
    max_steps = p2["max_train_steps"]
    save_every = p2.get("save_every_n_steps", 500)
    log_every = p2.get("log_every_n_steps", 100)

    print(f"Starting DreamBooth phase 2 for {max_steps} steps...")
    ...
    _save_checkpoint(cfg, accelerator, unet, pipe, global_step)
    print(f"DreamBooth phase 2 complete at step {global_step}.")
...
def _save_checkpoint(
    ...
    """Save combined LoRA weights (phase 1 + phase 2)."""
