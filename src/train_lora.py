#!/usr/bin/env python3
"""LoRA fine-tuning script for Stable Diffusion — attention layers only.

Patches self/cross-attention layers on UNet (to_q, to_k, to_v, to_out)
and text encoder (q_proj, k_proj, v_proj, out_proj) with LoRA adapters.

Corresponds to: notebooks/fine_tuning_phase_1.ipynb

Usage:
    python src/train_lora.py --config configs/training_config.yaml
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="LoRA fine-tuning (attention layers only)",
    )
    parser.add_argument(
...
def setup_pipeline(cfg: dict[str, Any], accelerator: Accelerator) -> tuple:
    """Load base model, tokenizer, UNet, and apply attention-only LoRA patches.
...
def train(cfg: dict[str, Any]) -> None:
    """Run the attention-only LoRA training loop."""
    enable_tf32()
    ...
    print(f"Starting attention-only LoRA training for {max_steps} steps...")
